...
Le projet est porté par 4 étudiant·es : 
* @pouvoirdasha
* @Sacha-N
* @jlechat 
* @xdalp / @mgarbe
...
## Conteneur docker : 
Pour faire tourner l'application rutabaga, le dockerfile présent dans le repo est pre-configuré pour avoir une version light du projet qui pèse environ 1 Go. La version dev du dockerfile avec uv donnerait un container de 12 Go : l'écart s'explique par les nombreuses dépendances utilisées pour nos transformations du plan de l'Ensae. Pour utiliser le docker (version light), il est possible d'utiliser la commande suivant pour build le container, depuis la racine du projet : 
```
docker build -t rutabaga-light .
```
Et ensuite, il est possible de le run et d'accéder au projet sur l'adresse http://localhost:5000/ : 
```
